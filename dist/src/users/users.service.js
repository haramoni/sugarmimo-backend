...
const profile_photo_thumbnail_1 = require("./profile-photo-thumbnail");
const proximity_ranking_1 = require("./proximity-ranking");
const relationship_intent_1 = require("./relationship-intent");
const UserRole = {
...
let UsersService = class UsersService {
    ...
    acceptPrivacyPolicy(id, version) {
        return this.prisma.user.update({
            where: { id },
            data: {
    ...
    async findBabiesForAdminFeaturing(page = 1, pageSize = 12, search) {
        const safePage = Number.isSafeInteger(page) && page > 0 ? Math.min(page, 100_000) : 1;
        const safePageSize = Number.isSafeInteger(pageSize) && pageSize > 0
    ...
    async findBabyPhotosForAdmin(id) {
    ...
    async updatePremiereStatus(id, isPremiere) {
        const profile = await this.prisma.user.findUnique({
            where: { id },
            select: { role: true },
        });
        ...
        if (profile.role !== UserRole.SugarDaddy) {
            throw new common_1.BadRequestException('O selo Premiere se aplica apenas a Sugar Daddies.');
        }
        return this.prisma.user.update({
            where: { id },
            data: { isPremiere },
    ...
    async updateAdminFeaturedStatus(id, isAdminFeatured) {
        const profile = await this.prisma.user.findUnique({
            where: { id },
        ...
        if (profile.role !== UserRole.SugarBaby ||
            ...
            throw new common_1.BadRequestException('A estrela se aplica apenas a Sugar Babies aprovadas e ativas.');
        }
        return this.prisma.user.update({
            where: { id },
    ...
    async removePendingProfilePhoto(userId, photoId) {
    ...
    async findMatchesForUser(viewerId, search, page = 1, limit = 9, minAge, maxAge, gender, latitude, longitude, relationshipMode) {
        ...
        const where = {
            ...
            approvalStatus: 'APPROVED',
            relationshipIntent: {
                in: (0, relationship_intent_1.resolveCompatibleRelationshipIntents)(viewer.relationshipIntent, relationshipMode),
            },
    ...
    async findDaddyMatchesWithBalancedRanking(where, viewerUsername, page, pageSize, pageStart, viewerLocation) {
        ...
        const orderedProfiles = pageIds
            ...
            .filter((profile) => Boolean(profile));
        return {
            items: orderedProfiles.map((profile) => this.sanitizePublicProfile(profile, viewerUsername)),
            page,
            pageSize,
            hasMore: pageStart + pageSize < rankedIds.length,
        };
    }
    buildBalancedDaddyRanking(candidates) {
        const now = Date.now();
        ...
        while (remaining.length > 0) {
            ...
            const matchIndex = remaining.findIndex((candidate) => {
                if (slot === 'online') {
                    return Boolean(candidate.lastActiveAt &&
                ...
                if (slot === 'new') {
                    return Boolean(candidate.createdAt && candidate.createdAt.getTime() >= newSince);
                }
                return true;
            });
    ...
    async findBoostedProfilesForUser(viewerId, page = 1, limit = 6) {
        ...
        const where = {
            ...
            approvalStatus: 'APPROVED',
            relationshipIntent: {
                in: (0, relationship_intent_1.resolveCompatibleRelationshipIntents)(viewer.relationshipIntent),
            },
    ...
    async findMatchPhotoForUser(viewerId, photoId, variant) {
        const viewer = await this.prisma.user.findUnique({
            where: { id: viewerId },
            select: {
                role: true,
        ...
        if (variant === 'card') {
            ...
            const cardDataUrl = photo.cardDataUrl ??
                (originalPhoto
                    ? await (0, profile_photo_thumbnail_1.createCardThumbnailDataUrl)(originalPhoto.dataUrl)
                    : undefined);
    ...
    async updateProfile(id, data) {
        ...
        if (data.profilePhotos) {
            ...
            preparedProfilePhotos = await Promise.all(data.profilePhotos.map(async (photo) => ({
                ...photo,
            ...
            await tx.user.update({
                ...
                data: {
                    ...
                    relationshipIntent: data.relationshipIntent === undefined
                        ? undefined
                        : (0, relationship_intent_1.normalizeRelationshipIntent)(data.relationshipIntent),
    ...
    matchDemographicFilter(viewerRole, minAge, maxAge, gender) {
        const conditions = [];
        if (viewerRole === UserRole.SugarDaddy) {
            conditions.push({
                OR: [
        ...
        const safeMinAge = Number.isFinite(minAge)
            ? Math.min(99, Math.max(18, Math.floor(minAge)))
            : undefined;
        const safeMaxAge = Number.isFinite(maxAge)
            ? Math.min(99, Math.max(18, Math.floor(maxAge)))
            : undefined;
        const birthDate = {};
