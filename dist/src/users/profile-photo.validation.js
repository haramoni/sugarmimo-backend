...
exports.validateProfilePhotos = validateProfilePhotos;
const common_1 = require("@nestjs/common");
const ALLOWED_IMAGE_TYPES = new Set(['image/jpeg', 'image/png', 'image/webp']);
const MAX_PHOTO_BYTES = 5 * 1024 * 1024;
const MAX_TOTAL_PHOTO_BYTES = 15 * 1024 * 1024;
function validateProfilePhotos(photos) {
    let totalBytes = 0;
    for (const photo of photos) {
        if (!photo.mimeType || !ALLOWED_IMAGE_TYPES.has(photo.mimeType)) {
            throw new common_1.BadRequestException('As fotos devem estar nos formatos JPEG, PNG ou WebP.');
        }
...
//# sourceMappingURL=profile-photo.validation.js.map