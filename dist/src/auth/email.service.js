...
let EmailService = class EmailService {
    ...
    async sendNewDaddyRegistration({ id, username, email, profileType, lookingFor, birthDate, country, state, city, whatsapp, telegram, instagram, occupation, source, referralUsername, createdAt, }) {
        const apiKey = this.configService.get('RESEND_API_KEY')?.trim();
        this.ensureConfigured();
        const value = (input) => input?.trim() || 'Nao informado';
        const response = await fetch('https://api.resend.com/emails', {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${apiKey}`,
